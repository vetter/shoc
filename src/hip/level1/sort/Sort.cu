        ...
        uint numElements)
{
    ...
    hipLaunchKernel(HIP_KERNEL_NAME(radixSortBlocks), dim3(radixBlocks), dim3(SORT_BLOCK_SIZE), 4 * sizeof(uint)*SORT_BLOCK_SIZE, 0, nbits, startbit, tempKeys, tempValues, keys, values);

    hipLaunchKernel(HIP_KERNEL_NAME(findRadixOffsets), dim3(findBlocks), dim3(SCAN_BLOCK_SIZE), 2 * SCAN_BLOCK_SIZE*sizeof(uint), 0, (uint2*)tempKeys, counters, blockOffsets, startbit, numElements,
         findBlocks);

    scanArrayRecursive(countersSum, counters, 16*reorderBlocks, 0,
            scanBlockSums);

    hipLaunchKernel(HIP_KERNEL_NAME(reorderData), dim3(reorderBlocks), dim3(SCAN_BLOCK_SIZE), 0, 0, startbit, (uint*)keys, (uint*)values, (uint2*)tempKeys,
        ...
        int level, uint** blockSums)
{
    ...
    if (numBlocks > 1)
    {
        hipLaunchKernel(HIP_KERNEL_NAME(scan), dim3(grid), dim3(threads), sharedMemSize, 0,
           outArray, inArray, blockSums[level], numElements, fullBlock, true);
    } else
    {
        hipLaunchKernel(HIP_KERNEL_NAME(scan), dim3(grid), dim3(threads), sharedMemSize, 0, outArray, inArray, blockSums[level], numElements, fullBlock, false);
    }
    if (numBlocks > 1)
    {
        scanArrayRecursive(blockSums[level], blockSums[level],
                numBlocks, level + 1, blockSums);
        hipLaunchKernel(HIP_KERNEL_NAME(vectorAddUniform4), dim3(grid), dim3(threads), 0, 0,                outArray, blockSums[level], numElements);
    }
}
