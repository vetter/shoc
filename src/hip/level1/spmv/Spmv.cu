        ...
        int numRows, int numNonZeroes, floatType* refOut, bool padded)
{
      ...
      for (int k=0; k<passes; k++)
      {
          ...
          {
              hipLaunchKernel(HIP_KERNEL_NAME(spmv_csr_scalar_kernel<floatType, texReader>), dim3(nBlocksScalar), dim3(BLOCK_SIZE), 0, 0, d_val, d_cols, d_rowDelimiters, numRows, d_out);
          }
          CUDA_SAFE_CALL(hipEventRecord(stop, 0));
          CUDA_SAFE_CALL(hipEventSynchronize(stop));
      ...
      hipLaunchKernel(HIP_KERNEL_NAME(zero<floatType>), dim3(nBlocksScalar), dim3(BLOCK_SIZE), 0, 0, d_out, numRows);
      ...
      for (int k=0; k<passes; k++)
      {
          ...
          for (int j = 0; j < iters; j++)
          {
              hipLaunchKernel(HIP_KERNEL_NAME(spmv_csr_vector_kernel<floatType, texReader>), dim3(nBlocksVector), dim3(BLOCK_SIZE), 0, 0, d_val, d_cols, d_rowDelimiters, numRows, d_out);
          }
          CUDA_SAFE_CALL(hipEventRecord(stop, 0));
          CUDA_SAFE_CALL(hipEventSynchronize(stop));
        ...
        int paddedSize)
{
    ...
    for (int k=0; k<passes; k++)
    {
        ...
        for (int j = 0; j < iters; j++)
        {
            hipLaunchKernel(HIP_KERNEL_NAME(spmv_ellpackr_kernel<floatType, texReader>), dim3(nBlocks), dim3(BLOCK_SIZE), 0, 0, d_val, d_cols, d_rowLengths, cmSize, d_out);
        }
        CUDA_SAFE_CALL(hipEventRecord(stop, 0));
        CUDA_SAFE_CALL(hipEventSynchronize(stop));
